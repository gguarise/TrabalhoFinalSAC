        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (logado.getNome() != null)
            {
                ...
                else if (action.equals("formAlterarFuncionario"))
                {
                    try
                    {
                        Funcionario funcionario = FuncionarioFacade.retornaFuncionario(Integer.parseInt(request.getParameter("idCadastrado")));
                ...
                else if (action.equals("alterarCadastro"))
                {
                    try
                    {
                       ...
                       if (tipo.equals("gerente"))
                       {
                           ...
                           {
                               request.setAttribute("msg", "Erro ao modificar gerente de id: " + gerente.getIdGerente());
                       ...
                       else if (tipo.equals("funcionario"))
                       {
                           ...
                           {
                               request.setAttribute("msg", "Erro ao modificar funcionario de id: " + funcionario.getIdFuncionario());
                               RequestDispatcher rd = request.getRequestDispatcher("/erro.jsp");
                               rd.forward(request, response);
                           }
                       }
                    }
                    catch(EstadoException | CidadeException | GerenteException | FuncionarioException | NumberFormatException e)
                    {
                        request.setAttribute("msg", "ERRO: " + e.getMessage());
                        RequestDispatcher rd = request.getRequestDispatcher("/erro.jsp");
                        rd.forward(request, response);
                       ...
                       else if (tipo.equals("funcionario"))
                       {
                           ...
                           else
                           {
                               ...
                               Funcionario novoFuncionario = FuncionarioFacade.adicionaFuncionario(funcionario);
                                ...
                                {
                                    request.setAttribute("msg", "Erro ao modificar gerente de id: " + funcionario.getIdFuncionario());
                                    RequestDispatcher rd = request.getRequestDispatcher("/erro.jsp");
                                    rd.forward(request, response);
                                }
                           }
                ...
                else if (action.equals("atendimentosAbertos"))
                {
                    try
                    {
                        ...
                        {
                            Collections.sort(listaAtendimentosAbertos, (Atendimento a1, Atendimento a2) -> a1.getDataHoraInicio().compareTo(a2.getDataHoraFim()));
